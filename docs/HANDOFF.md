...
- **State:** 211 tests passing (1 + 124 subset-lib + 30 integration
  + 56 vc), both lanes warning-clean, clippy error-clean. Done:
...
| Lane | Status |
|---|---|
| `cargo +stable test --workspace --all-features` | **211 passing**, 0 failed, 0 ignored, 0 warnings |
...
### Step 4.2 — Stable test suite (the 211-test baseline)

```bash
cargo +stable test --workspace --all-features 2>&1 | grep "^test result"
# Expected: "test result: ok" lines totaling 211 passing, 0 failed, 0 ignored
```
...
# Expected: all integration tests run (none gracefully skipped). Still 211 passing.
...
# Expected: 211 passing (same as without Z3 — the new tests
