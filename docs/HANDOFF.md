...
- **State:** 328 tests passing (186 subset-lib + 70 vc + 53 integration + 11 aorte_proofs + 8 driver-bin),
...
| Lane | Status |
|---|---|
| `cargo +stable test --workspace --all-features` | **328 passing**, 0 failed, 0 ignored, 0 warnings |
...
The **328** breaks down: 1 (cargo-pitbull bin) + 7 (pitbull-rustc bin) + 186
(subset lib) + 53 (integration) + 11 (aorte_proofs) + 70 (vc) = 328. This supersedes the long
...
### Step 4.2 — Stable test suite (the 328-test baseline)

```bash
cargo +stable test --workspace --all-features 2>&1 | grep "^test result"
# Expected: "test result: ok" lines totaling 328 passing, 0 failed, 0 ignored
```
...
# Expected: all integration tests run (none gracefully skipped). Still 328 passing.
...
# Expected: 328 passing (same as without Z3 — the new tests
