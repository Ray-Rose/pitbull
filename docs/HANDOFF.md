...
- **State:** 316 tests passing (182 subset-lib + 70 vc + 56 integration + 8 driver-bin),
...
| Lane | Status |
|---|---|
| `cargo +stable test --workspace --all-features` | **316 passing**, 0 failed, 0 ignored, 0 warnings |
...
The **316** breaks down: 1 (cargo-pitbull bin) + 7 (pitbull-rustc bin) + 182
(subset lib) + 56 (integration) + 70 (vc) = 316. This supersedes the long
...
### Step 4.2 — Stable test suite (the 316-test baseline)

```bash
cargo +stable test --workspace --all-features 2>&1 | grep "^test result"
# Expected: "test result: ok" lines totaling 316 passing, 0 failed, 0 ignored
```
...
# Expected: all integration tests run (none gracefully skipped). Still 316 passing.
...
# Expected: 316 passing (same as without Z3 — the new tests
