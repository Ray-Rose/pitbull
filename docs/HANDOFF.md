...
- **State:** 330 tests passing (187 subset-lib + 70 vc + 54 integration + 11 aorte_proofs + 8 driver-bin),
...
| Lane | Status |
|---|---|
| `cargo +stable test --workspace --all-features` | **330 passing**, 0 failed, 0 ignored, 0 warnings |
...
The **330** breaks down: 1 (cargo-pitbull bin) + 7 (pitbull-rustc bin) + 187
(subset lib) + 54 (integration) + 11 (aorte_proofs) + 70 (vc) = 330. This supersedes the long
...
### Step 4.2 — Stable test suite (the 330-test baseline)

```bash
cargo +stable test --workspace --all-features 2>&1 | grep "^test result"
# Expected: "test result: ok" lines totaling 330 passing, 0 failed, 0 ignored
```
...
# Expected: all integration tests run (none gracefully skipped). Still 330 passing.
...
# Expected: 330 passing (same as without Z3 — the new tests
