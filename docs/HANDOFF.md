...
- **State:** 324 tests passing (183 subset-lib + 70 vc + 52 integration + 11 aorte_proofs + 8 driver-bin),
...
| Lane | Status |
|---|---|
| `cargo +stable test --workspace --all-features` | **324 passing**, 0 failed, 0 ignored, 0 warnings |
...
The **324** breaks down: 1 (cargo-pitbull bin) + 7 (pitbull-rustc bin) + 183
(subset lib) + 52 (integration) + 11 (aorte_proofs) + 70 (vc) = 324. This supersedes the long
...
### Step 4.2 — Stable test suite (the 324-test baseline)

```bash
cargo +stable test --workspace --all-features 2>&1 | grep "^test result"
# Expected: "test result: ok" lines totaling 324 passing, 0 failed, 0 ignored
```
...
# Expected: all integration tests run (none gracefully skipped). Still 324 passing.
...
# Expected: 324 passing (same as without Z3 — the new tests
