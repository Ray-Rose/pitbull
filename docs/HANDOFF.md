...
- **State:** 321 tests passing (182 subset-lib + 70 vc + 51 integration + 10 aorte_proofs + 8 driver-bin),
...
| Lane | Status |
|---|---|
| `cargo +stable test --workspace --all-features` | **321 passing**, 0 failed, 0 ignored, 0 warnings |
...
The **321** breaks down: 1 (cargo-pitbull bin) + 7 (pitbull-rustc bin) + 182
(subset lib) + 51 (integration) + 10 (aorte_proofs) + 70 (vc) = 321. This supersedes the long
...
### Step 4.2 — Stable test suite (the 321-test baseline)

```bash
cargo +stable test --workspace --all-features 2>&1 | grep "^test result"
# Expected: "test result: ok" lines totaling 321 passing, 0 failed, 0 ignored
```
...
# Expected: all integration tests run (none gracefully skipped). Still 321 passing.
...
# Expected: 321 passing (same as without Z3 — the new tests
